...
from lib.dataset.utils import transform_coord, transform_coord_tensor
...
class FormatDataTensor:
    ...
    def __call__(self, data, predictions):
        ...
        for i, name in enumerate(names):
            ...
            # Renormalized the predicted traj
            _pred_traj = transform_coord_tensor(pred_trajs[i], -theta[i])
            _pred_traj = _pred_traj + togloble[i].cuda()

            # Renormalized groundtruth
            gt = data['FUTURE'][i][0,:,:2]
            gt = gt.cumsum(axis=-2)
            gt = transform_coord_tensor(gt, -theta[i])
            gt = gt.cuda() + togloble[i].cuda()
